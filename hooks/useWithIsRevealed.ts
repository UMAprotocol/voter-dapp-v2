...
export default function useWithIsRevealed(votingContract: VotingV2Ethers, address: string, votes: VoteT[] | null) {
  ...
  const withIsRevealed = votes?.map((vote) => ({ ...vote, isRevealed: keys.includes(vote.uniqueKey) }));
