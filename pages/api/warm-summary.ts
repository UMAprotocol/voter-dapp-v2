...
const ORACLE_CHILD_TUNNEL_ADDRESS = "0xac60353a54873c446101216829a6A98cDbbC3f3D";
const BATCH_SIZE = 5;
...
function decodeAncillaryData(ancillaryData: string): string {
  try {
    ...
    const cleanHex = ancillaryData.startsWith('0x') ? ancillaryData.slice(2) : ancillaryData;
    ...
    if (titleStart !== -1 && descriptionStart !== -1 && descriptionStart > titleStart) {
    ...
    if (beforeComma && beforeComma.length > 5) {
      let title = beforeComma.replace(/\+/g, ' ');
      console.log(`📝 Using text before comma: "${title}" from decoded: "${decoded}"`);
      return title;
    }
...
function buildUpdateSummaryUrl(event: PriceRequestEvent, title: string): string {
  const baseUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/update-summary`;
  ...
  return `${baseUrl}?time=${event.time.toString()}&identifier=${event.identifier}&title=${encodedTitle}`;
}
...
async function batchUpdateSummaries(urls: string[]): Promise<{
  ...
  const promises = urls.map(async (url) => {
    try {
      ...
      console.log(`✅ Success: ${url} - Updated: ${result.updated}, Cached: ${result.cached}`);
      successful.push(url);
    } catch (error) {
...
export default async function handler(
  ...
  if (!process.env.NODE_URLS) {
    return res.status(500).json({ error: "NODE_URLS environment variable not set" });
  }
  
  let nodeUrls: any;
  try {
    nodeUrls = JSON.parse(process.env.NODE_URLS);
  } catch (error) {
    return res.status(500).json({ error: "Invalid NODE_URLS format - must be valid JSON" });
  }
  
  const polygonRpcUrl = nodeUrls["137"]; // Chain ID 137 is Polygon
  
  if (!polygonRpcUrl) {
    return res.status(500).json({ error: "Polygon RPC URL not configured in NODE_URLS (missing chain ID 137)" });
  }
  ...
  try {
    ...
    const contractInterface = new ethers.utils.Interface(PRICE_REQUEST_BRIDGED_ABI);
    ...
    for (const log of logs.slice(-MAX_EVENTS)) { // Take most recent events
      try {
        ...
        const event: PriceRequestEvent = {
          ...
          time: parsed.args.time,
          ancillaryData: parsed.args.ancillaryData,
          childRequestId: parsed.args.childRequestId,
          parentRequestId: parsed.args.parentRequestId,
        ...
        console.warn(`⚠️  Failed to parse event in block ${log.blockNumber}:`, parseError);
      }
    }
    
    console.log(`✅ Parsed ${parsedEvents.length} valid events (filtered for last ${MAX_AGE_DAYS} days)`);
    ...
    for (const event of parsedEvents) {
      try {
        ...
        updateUrls.push(url);
        console.log(`📝 Event ${event.childRequestId.slice(0, 10)}... -> "${title}"`);
      } catch (error) {
    ...
    console.log(`🚀 Processing ${updateUrls.length} URLs in batches of ${BATCH_SIZE}...`);
    ...
    for (let i = 0; i < updateUrls.length; i += BATCH_SIZE) {
      ...
      console.log(`✅ Batch ${batchNumber} complete: ${successful.length} success, ${errors.length} errors`);
